...
export default function Question5() {
  ...
  useEffect(()=>{
    if(question5=="" && localStorage.getItem("Question5")){
     setQuestion5(localStorage.getItem("Question5"))
    }
},[])
  const sendMessage = async () => {
    console.log(question5)
    const input = `
Question: How does the local regulatory and policy environment impact the economic feasibility of the technology or project? 
(e.g., supportive energy policies, tax policies, regulations for clean energy or innovation) 
...
`;
    ...
    try {
      ...
      localStorage.setItem("Answer5", answer.data.choices[0].message.content);
  ...
  <CircularProgress  sx={{ color: "#0e8d93" }} size="lg" determinate value={57.2}>
        <p style={{color:'whitesmoke'}}>57.2%</p>
      </CircularProgress>
      </div>
...
<div  id='container3' style={{borderRadius:'10px',backgroundColor:'#ececec',marginTop:'100px'}}>  <div>
    <h4 style={{color:'#139ca3'}}className='question'>Question #5</h4>
    <p>How does the local regulatory and policy environment impact the economic feasibility of the technology or project? (e.g., supportive energy policies, tax policies, regulations for clean energy or innovation)</p>
    <h4 className='question'style={{color:'#139ca3'}}>Answer</h4>
    <Textarea minRows={5} style={{maxWidth:'100%'}} value={question5} onChange={handleQuestion5Change}/>
                <br/>
      </div>
      <div class="links">
      <button className="follow" onClick={() => navigate('/Question4')}>Back</button>
