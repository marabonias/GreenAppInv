...
export default function Question6() {
  ...
  useEffect(()=>{
    if(question6=="" && localStorage.getItem("Question6")){
     setQuestion6(localStorage.getItem("Question6"))
    }
},[])
  const sendMessage = async () => {
    console.log(question6)
    const input = `
Question: What are the upfront and ongoing costs of implementing the technology, and how do they compare to the long-term benefits, particularly for vulnerable communities? 
...
`;
    ...
    try {
      ...
      localStorage.setItem("Answer6", answer.data.choices[0].message.content);
  ...
  <CircularProgress  sx={{ color: "#0e8d93" }} size="lg" determinate value={64.3}>
        <p style={{color:'whitesmoke'}}>64.3%</p>
      </CircularProgress>
      </div>
...
<div  id='container3' style={{borderRadius:'10px',backgroundColor:'#ececec',marginTop:'100px'}}>  <div>
    <h4 style={{color:'#139ca3'}}className='question'>Question #6</h4>
    <p>What are the upfront and ongoing costs of implementing the technology, and how do they compare to the long-term benefits, particularly for vulnerable communities?</p>
    <h4 className='question'style={{color:'#139ca3'}}>Answer</h4>
    <Textarea minRows={5} style={{maxWidth:'100%'}} value={question6} onChange={handleQuestion6Change}/>
                <br/>
      </div>
      <div class="links">
      <button className="follow" onClick={() => navigate('/Question5')}>Back</button>
