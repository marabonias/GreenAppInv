...
export default function Question9() {
  ...
  useEffect(()=>{
    if(question9=="" && localStorage.getItem("Question9")){
     setQuestion9(localStorage.getItem("Question9"))
    }
},[])
  ...
  const sendMessage = async () => {
    ...
    const input =  `
    Question: How does the technology foster long-term community engagement and empowerment? 
    ...
    try {
      ...
      localStorage.setItem("Answer9", answer.data.choices[0].message.content);
  ...
  <CircularProgress  sx={{ color: "#0e8d93" }} size="lg" determinate value={85.8}>
        <p style={{color:'whitesmoke'}}>85.8%</p>
      </CircularProgress>
      </div>
...
<div  id='container3' style={{borderRadius:'10px',backgroundColor:'#ececec',marginTop:'100px'}}>  <div>
    <h4 style={{color:'#139ca3'}}className='question'>Question #9</h4>
    <p>How does the technology foster long-term community engagement and empowerment?</p>
    <h4 className='question'style={{color:'#139ca3'}}>Answer</h4>
    <Textarea minRows={5} style={{maxWidth:'100%'}} value={question9} onChange={handleQuestion9Change}/>
                <br/>
      </div>
      <div class="links">
      <button className="follow" onClick={() => navigate('/Question8')}>Back</button>
